...
public class MorphologicFilter implements IImageProcessor {
    ...
    public ImageData run(ImageData inData, int imageType) {
        ...
        switch (ch) {
            ...
            case 2:
                return opening(inData, struct, cx, cy, 1);
            case 3:
                return closing(inData, struct, cx, cy, 1);
            case 4:
    ...
    public static ImageData erosion(ImageData inData, boolean[][] struct, int cx, int cy) {
        ...
        Parallel.For(0, outData.height, v -> {
            for (int u = 0; u < outData.width; u++) {
                ...
                if (set) outData.setPixel(u, v, s_foreground);
                else outData.setPixel(u, v, s_background);
            }
        });
        return outData;
    }
     ...
     */
    public static ImageData opening(ImageData inData, boolean[][] struct, int cx, int cy, int iterations) {
        ImageData temp = inData;
        for (int k = 0; k < iterations; k++) {
            temp = erosion(temp, struct, cx, cy);
        }
        for (int k = 0; k < iterations; k++) {
            temp = dilation(temp, struct, cx, cy);
        }
        return temp;
    }
     ...
     */
    public static ImageData closing(ImageData inData, boolean[][] struct, int cx, int cy, int iterations) {
        ImageData temp = inData;
        for (int k = 0; k < iterations; k++) {
            temp = dilation(temp, struct, cx, cy);
        }
    ...
    private static ImageData subtract(ImageData imageA, ImageData imageB) {
        ImageData result = (ImageData) imageA.clone();
        for (int y = 0; y < result.height; y++) {
            for (int x = 0; x < result.width; x++) {
                if (imageA.getPixel(x, y) == s_foreground && imageB.getPixel(x, y) == s_background) {
                    result.setPixel(x, y, s_foreground);
                } else {
                    result.setPixel(x, y, s_background);
                }
            }
        }
        return result;
    ...
    public static ImageData contour(ImageData inData, boolean[][] struct, int cx, int cy, boolean inner) {
        if (inner) {
            ImageData eroded = erosion(inData, struct, cx, cy);
            return subtract(inData, eroded);
        } else {
            ImageData dilated = dilation(inData, struct, cx, cy);
            return subtract(dilated, inData);
        }
    }
