...
//        Matrix inverseTransformationMatrix = transformationMatrix.inverse();
        ...
        Matrix t = Matrix.translation(outData.width / 2, outData.height / 2)
                .multiply(Matrix.rotation(Math.toRadians(rotation)))
                .multiply(Matrix.translation(-inData.width / 2, -inData.height / 2))
                .inverse();
        ...
        Parallel.For(0, outData.width, i -> {
            for (int j = 0; j < outData.height; j++) {
                ...
                Matrix newPoint = t.multiply(point);
